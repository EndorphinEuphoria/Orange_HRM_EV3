...
public class PimPage {
    ...
    public void iniciarSesionAdmin() {
        ...
        driver.findElement(campoPassword)
                .sendKeys("admin123");
    ...
    public void clickPim() {
        wait.until(ExpectedConditions.elementToBeClickable(menuPim))
                .click();
    }

    public boolean estaEnPim() {
        return wait.until(
                ExpectedConditions.visibilityOfElementLocated(tituloPim)
        ).isDisplayed();
    }

    private By inputEmployeeName =
        By.xpath("(//input[@placeholder='Type for hints...'])[1]");

    private By botonSearch =
    ...
    public void buscarEmpleado(String nombre) {
        ...
        wait.until(ExpectedConditions.visibilityOfElementLocated(inputEmployeeName))
            .sendKeys(nombre);

        wait.until(ExpectedConditions.elementToBeClickable(botonSearch))
            .click();
    }
    ...
    try {
        ...
        new WebDriverWait(driver, Duration.ofSeconds(10))
            .until(ExpectedConditions.or(
                ...
                ExpectedConditions.visibilityOfElementLocated(
                    By.xpath("//p[text()='No Records Found']")
                ),
                ExpectedConditions.visibilityOfElementLocated(
                    By.cssSelector(".oxd-toast--info")
                )
            ));
        return true;
    } catch (Exception e) {
